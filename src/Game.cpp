...
gv(std::make_unique<GameView>(std::vector<std::string>{"images/maze-a.png","images/maze-b.png","images/maze-c.png","images/maze-d.png"})) {
	...
	ghostsControl.push_back(std::make_shared<InkyController>(ghosts[1])); //azul
	ghostsControl.push_back(std::make_shared<PinkyController>(ghosts[2])); //rosado
	ghostsControl.push_back(std::make_shared<RandomController>(ghosts[3])); //naranja
